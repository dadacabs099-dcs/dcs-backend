...
class PlaceDetailsRepo {
  ...
  Future<dynamic> getAllPredictedPlaceDetails(
    ...
    try {
      ...
      String url =
          "https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&key=${AppKeys.mapKey}";

      Response res = await Dio().get(url);

      if (res.statusCode == 200) {
        Direction placeDetails = Direction(
          locationName: res.data["result"]["formatted_address"] ?? res.data["result"]["name"],
          locationId: placeId,
        ...
        // Clear the stale search suggestions after a successful selection.
        ref.read(whereToPredictedPlacesProvider.notifier).update((state) => null);
