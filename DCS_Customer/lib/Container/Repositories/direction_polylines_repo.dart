...
class DirectionPolylines {
  ...
  Future<dynamic> getDirectionsPolylines(context, WidgetRef ref) async {
    try {
      ...
      String url =
          "https://maps.googleapis.com/maps/api/directions/json?origin=${pickUpDestination.latitude},${pickUpDestination.longitude}&destination=${dropOffDestination.latitude},${dropOffDestination.longitude}&key=${AppKeys.mapKey}";
  ...
  Future<dynamic> calculateRideRate(context, WidgetRef ref) async {
    try {
      ...
      // Save direction details model in provider
      ref
          .read(homeScreenDirectionDetailsProvider.notifier)
          .update((state) => model);
